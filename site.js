...
/**
 * Fills the empty left/right gutters outside the content column with a
 * faint, slow digital-rain drift -- purely decorative, canvas-based, and
 * only ever drawn in the gutters themselves (never over content). Skipped
 * entirely on touch devices and when the viewport is too narrow to have a
 * meaningful gutter. Reduced-motion draws one static frame instead of
 * animating, matching the site's other motion opt-outs.
 */
function initMatrixRain() {
  if (window.matchMedia('(pointer: coarse)').matches) return;

  var CONTENT_MAX = 1240;
  var MIN_GUTTER = 90;
  var FONT_SIZE = 15;
  var STEP_MS = 90;
  ...
  function makeCanvas(side) {
    ...
    c.className = 'matrix-rain matrix-rain-' + side;
  ...
  var sides = [
    { canvas: makeCanvas('left'), ctx: null, drops: [] },
    { canvas: makeCanvas('right'), ctx: null, drops: [] }
  ];
  sides.forEach(function (s) { s.ctx = s.canvas.getContext('2d'); });

  var gutter = 0;

  function setup() {
    gutter = (window.innerWidth - CONTENT_MAX) / 2;
    var show = gutter >= MIN_GUTTER;
    sides.forEach(function (s) {
      s.canvas.style.display = show ? 'block' : 'none';
    });
    if (!show) return;
    ...
    sides.forEach(function (s) {
      ...
      s.canvas.style.width = gutter + 'px';
  ...
  function drawFrame() {
    ...
    var h = window.innerHeight;
    sides.forEach(function (s) {
      s.ctx.fillStyle = 'rgba(0,0,0,0.07)';
      s.ctx.fillRect(0, 0, gutter, h);
