...
- Any "save," "export," or "download" feature (Copy-as-markdown, Print / Save as PDF, and anything added later in this family) generates its output entirely client-side and triggers a direct browser download, print, or clipboard write — no server round-trip, no email capture, no data collected anywhere. This is a firm precedent for the whole site, not a case-by-case call: it's the same "nothing leaves the browser unless you explicitly ask for the BYOK AI extra" guarantee applied to exports specifically. Prefer the browser's own native mechanism (print stylesheet, clipboard API) over a third-party library approximating it — see the Print/PDF history below for why.
...
- **Print / Save as PDF**: `post.html` includes a "Print / Save as PDF" button below the next-post box. `setupPrintPdfButton()` in `site.js` just calls `window.print()` — all styling comes from the `@media print` block at the end of `style.css`. This is the *third* implementation of this feature, and the history matters: v1 (html2canvas/html2pdf.js rasterizing a styled DOM clone) and v2 (jsPDF drawing the page manually with embedded webfonts) both went through multiple rounds of real-world blank/broken PDFs that never reproduced in testing, because both approaches were really just approximating what the browser's own rendering engine already does correctly. `window.print()` + a print stylesheet uses that real engine directly, so there's no approximation left to get wrong — this is now the standing approach for any future "make the page a PDF" work; don't reintroduce a rasterization/rebuild library without a very specific reason. The print stylesheet forces `print-color-adjust:exact` (browsers strip background colors by default when printing) and forces `.reveal`/`.declass` fully visible (those classes default to hidden until scrolled into view on screen, which shouldn't gate what prints), hides anything that's browser-chrome/navigation rather than article content (nav, TOC, buttons, footer, next-post teaser, related-reads, glossary nudge), and **must also hide `.boot-overlay`** — the one-time "security checkup" boot animation is `position:fixed; z-index:200` covering the full viewport, and printing while it's still visible (or mid-animation) would otherwise blank out the whole page.
