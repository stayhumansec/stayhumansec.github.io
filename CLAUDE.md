...
- **Download as PDF**: `post.html` includes a "Download as PDF" button below the next-post box. `generatePostPdf()` in `site.js` draws the PDF directly with jsPDF's native text/shape API (dark background fill, cream/orange palette, terminal-chrome header bar, built-in helvetica/courier fonts) — deliberately **not** a DOM screenshot. An earlier html2canvas/html2pdf.js version (rasterizing a styled DOM clone) went through three rounds of real-world blank-PDF bugs that never reproduced in testing, because html2canvas's cloning/rendering pipeline was an unreliable black box across browsers. Native jsPDF drawing is deterministic and has no such failure class. `setupDownloadPdfButton()` lazy-loads jsPDF from a CDN only on first click and triggers a direct download — entirely client-side, no server, no email, nothing collected. Same free-for-every-post guarantee as Copy-as-markdown. Any text reaching jsPDF's built-in fonts must go through `sanitizePdfText()` first — those fonts only support the WinAnsi glyph set, so Unicode symbols (arrows, checkmarks, etc.) used in post content or this file's own markup need plain-ASCII equivalents or they render as garbage and corrupt jsPDF's line-wrapping width math.
